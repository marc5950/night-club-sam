		...
		<footer
			...
			<div className="max-w-[1440px] flex flex-col gap-12">
				<div className=" w-full grid grid-cols-1 md:grid-cols-3 gap-8">
					...
					<div className="flex flex-col gap-6 items-center md:items-start">
						...
						<div>
							...
							<p className="text-primary text-lg">WED - THU: 10:30 PM TO 3 AM</p>
							<p className="text-primary text-lg">SAT - SUN: 11 PM TO 5 AM</p>
						</div>
					</div>
							...
							{recentPosts.map((post) => (
								<div key={post.id} className="flex gap-3">
									...
									<div className="flex flex-col gap-2">
										<h3 className="text-primary text-lg font-medium">{post.title}</h3>
						...
						<div className="flex flex-col gap-6">
							{recentTweets.map((tweet) => (
								<div key={tweet.id} className="flex gap-3">
									...
									<div>
										<p className="text-primary text-lg">{tweet.text}</p>
				...
				<div className="flex flex-col md:flex-row justify-between  md:items-end gap-6 items-center">
					<p className="text-lg max-w-[250px] text-primary">Night Club PSD Template - All rights reserved.</p>
					<div className="flex flex-col items-center">
						<h3 className="text-lg text-primary">Stay Connected With Us</h3>
						<div className="flex gap-4 mt-2 text-primary">
							<FaFacebookF className="border cursor-pointer border-primary p-2 h-12 w-12" />
							<FaSnapchatGhost className="border cursor-pointer border-primary p-2 h-12 w-12" />
							<FaInstagram className="border cursor-pointer border-primary p-2 h-12 w-12" />
						</div>
					</div>
					<p className="text-lg max-w-[250px] text-end text-primary">Copyright &copy; {new Date().getFullYear()} NightClub</p>
				</div>
			</div>
		</footer>
	);
